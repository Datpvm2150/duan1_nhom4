...
{
    // Bước 1: Kiểm tra xem id_quan_tri có tồn tại và hợp lệ không
    if (!isset($_GET['id_quan_tri']) || !is_numeric($_GET['id_quan_tri'])) {
        echo "Lỗi: ID tài khoản không hợp lệ!";
        return;
    }

    // Bước 2: Lấy ID và ép kiểu thành số nguyên
    $tai_khoan_id = (int)$_GET['id_quan_tri'];

    // Bước 3: Lấy thông tin tài khoản
    $tai_khoan = $this->modelTaiKhoan->getDetailTaiKhoan($tai_khoan_id);

    // Bước 4: Kiểm tra xem tài khoản có tồn tại không
    if ($tai_khoan === false || !is_array($tai_khoan)) {
        echo "Lỗi: Tài khoản không tồn tại!";
        return;
    }

    // Bước 5: Kiểm tra xem tài khoản có phải quản trị không
    if ($tai_khoan['chuc_vu_id'] != 1) {
        echo "Lỗi: Chỉ có thể reset mật khẩu cho tài khoản quản trị!";
        return;
    }
    ...
    // Bước 7: Kiểm tra kết quả reset và redirect
    if ($status) {
        header("Location: " . BASE_URL_ADMIN . '?act=tai-khoan-quan-tri');
        exit();
    }

    // Bước 8: Thông báo lỗi nếu reset thất bại
    echo "Lỗi: Không thể reset mật khẩu!";
}
